...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  function handleResponse(connection, err, result) {
    // We behave badly and propagate transactions into models that use different
    // connectors.
    if (!transaction || (transaction && transaction.connection !== connection)) {
      connection.release();
    }
...
MySQL.prototype.toColumnValue = function(prop, val) {
  ...
  // Support datelike
  if(val.getUTCFullYear && val.getUTCMonth && val.getUTCDate && val.getUTCHours && val.getUTCMinutes && val.getUTCSeconds) {
